...
window.DashboardAPI = (function() {
    ...
    return {
        ...
        // S3 Backup APIs
        async triggerS3Backup() {
            const res = await fetch(`${API}/s3/backup`, { method: 'POST' });
            return res.json();
        },

        async listS3Backups() {
            try {
                const res = await fetch(`${API}/s3/backups`);
                const data = await res.json();
                return data.ok ? (data.data || []) : [];
            } catch (e) {
                console.error('S3 backups list error:', e);
                return [];
            }
        },

        async restoreS3Backup(key) {
