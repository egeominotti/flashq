/**
 * Production Test: 100,000 Jobs - HIGH CONCURRENCY
 *
 * Run: bun run examples/production-100k-highconcurrency.ts
 */

import { FlashQ, Worker } from '../src';

const QUEUE_NAME = 'production-100k-hc';
const TOTAL_JOBS = 100_000;
const BATCH_SIZE = 1000;
const WORKER_CONCURRENCY = 200;
const NUM_WORKERS = 8;

async function main() {
  console.log('\n╔════════════════════════════════════════════════════════╗');
  console.log('║   PRODUCTION TEST: 100K JOBS - HIGH CONCURRENCY        ║');
  console.log('╚════════════════════════════════════════════════════════╝\n');
  ...
  // Start all workers
  console.log(`Starting ${NUM_WORKERS} workers (${NUM_WORKERS * WORKER_CONCURRENCY} total concurrency)...\n`);
