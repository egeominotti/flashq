...
async function runBenchmark(runNumber: number): Promise<RunResult> {
  ...
  console.log(`\n${'='.repeat(70)}`);
  console.log(`🚀 RUN ${runNumber}/${NUM_RUNS} - ${TOTAL_JOBS.toLocaleString()} Jobs`);
  ...
  console.log(`✅ Push complete: ${pushRate.toLocaleString()} jobs/sec (${(pushTime/1000).toFixed(2)}s)`);
  ...
  const progressInterval = setInterval(() => {
    ...
    console.log(`   ${processed.toLocaleString()}/${TOTAL_JOBS.toLocaleString()} (${pct}%) | ${currentRate.toLocaleString()}/s | Avg: ${avgRate.toLocaleString()}/s`);
  ...
  while (processed + errors < TOTAL_JOBS) {
    if (Date.now() > timeout) {
      console.error(`❌ TIMEOUT: Only ${processed + errors}/${TOTAL_JOBS} jobs completed`);
      break;
    }
  ...
  if (!allAccountedFor) {
    console.error(`❌ MISSING JOBS: ${totalHandled}/${TOTAL_JOBS} (${TOTAL_JOBS - totalHandled} lost)`);
  }
...
const avgTotalTime = results.reduce((s, r) => s + r.totalTime, 0) / results.length;
...
console.log('');
console.log(`Process rate range: ${minProcessRate.toLocaleString()}/s - ${maxProcessRate.toLocaleString()}/s`);
console.log(`Total jobs pushed: ${(TOTAL_JOBS * NUM_RUNS).toLocaleString()}`);
console.log(`Total processed: ${totalProcessed.toLocaleString()}`);
if (totalErrors > 0) console.log(`Total errors: ${totalErrors.toLocaleString()}`);
if (totalDataErrors > 0) console.log(`Total data errors: ${totalDataErrors.toLocaleString()}`);
if (totalMissing > 0) console.log(`Total missing: ${totalMissing.toLocaleString()}`);
